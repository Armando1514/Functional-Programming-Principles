...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction { as =>
      as match
        case Json.Arr(items) => Some(items.map(_.decodeAs[A]).map(_.get))
        case _               => Some(List(as.decodeAs[A].get))
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction(obj =>
      obj match
        case Json.Obj(map) => decoder.decode(map(name))
        case _             => None
    )
