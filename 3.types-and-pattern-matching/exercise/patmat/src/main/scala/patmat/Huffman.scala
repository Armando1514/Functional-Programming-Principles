...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    @tailrec
    def occurrencesCalculation(char: Char, chars: List[Char], occ: Int): Int =

      if chars.isEmpty
      then occ
      else chars.head match {
        case `char` => occurrencesCalculation(char, chars.tail,  occ + 1)
        case _ => occurrencesCalculation(char, chars.tail, occ)
      }
    end occurrencesCalculation


    if !chars.isEmpty then {
      val occ = occurrencesCalculation(chars.head, chars.tail, 1)
      (chars.head, occ) :: times(chars.filter(x => x != chars.head))
    }
    else Nil
  ...
  def singleton(trees: List[CodeTree]): Boolean =
    if trees.isEmpty then false
    else if trees.tail.isEmpty then true
    else false
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def discover(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
         case Leaf(c, _) =>
            if bits.isEmpty then List(c)
            else c :: discover(tree, bits)

          case Fork(left, right, _, _) =>
            if bits.head == 0 then discover(left, bits.tail)
            else discover(right, bits.tail)
        }
    discover(tree, bits)
