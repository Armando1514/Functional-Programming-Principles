...
abstract class TweetSet extends TweetSetInterface:
   ...
   *
   * Calling `Retweeted` on an empty set should throw an exception of
  ...
  def descendingByRetweet: TweetList =
    this match {
      // case no left node, so explore to right
      case empty: Empty => Nil
      case nonEmpty: NonEmpty =>
        val mostretweeted = mostRetweeted
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    ...
    left match {
      // case no left node, so explore to right
      case empty : Empty => right match {
        // if right is empty, end of the tree, return the biggest elem
        case empty : Empty => elem
        // if right is not empty, find the max between current node and right node
        case nonEmpty: NonEmpty => MaxRetweeted(elem, right.mostRetweeted)
      }
      // case left node not empty, so explore right
      case nonEmpty: NonEmpty =>  right match {
        // if right empty return elem
        case empty: Empty => elem
        // if right not empty as well, check the max between both
        case nonEmpty: NonEmpty => MaxRetweeted(left.mostRetweeted,MaxRetweeted(elem, left.mostRetweeted))
      }
    }
