...
trait FunSets extends FunSetsInterface:
  ...
  def map(s: FunSet, f: Int => Int): FunSet = {
    def iterate(s: FunSet, accumulate: FunSet, a:Int): FunSet = {
      if (a > bound) accumulate
      else if (contains(s, a)) iterate(s, union(accumulate, x=>f(a) == x), a+1)
      else iterate(s, accumulate, a+1)
    }
